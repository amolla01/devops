...
Actions:
  ...
  infra-services          Run kubespray + metallb + postgresql + ceph + openstack + monitoring
...
sync_remote_v13_script() {
  local remote_target="$1"
  local target_path="$2"
  local remote_home="$3"
  local local_v13="$SCRIPT_DIR/deploy_lab_v13.sh"

  [[ -f "$local_v13" ]] || die "deploy_lab_v13.sh not found locally ($local_v13)"

  local should_copy=false
  if ! ssh "${SSH_OPTS[@]}" "$remote_target" "test -f '$target_path'" >/dev/null 2>&1; then
    warn "deploy_lab_v13.sh not found at $target_path on $remote_target"
    should_copy=true
  else
    local local_sha remote_sha
    local_sha="$(sha256sum "$local_v13" | awk '{print $1}')"
    remote_sha="$(ssh "${SSH_OPTS[@]}" "$remote_target" "sha256sum '$target_path' 2>/dev/null | awk '{print \\$1}'" 2>/dev/null || true)"
    if [[ -z "$remote_sha" || "$remote_sha" != "$local_sha" ]]; then
      log "Remote deploy_lab_v13.sh differs from local copy; syncing latest version..."
      should_copy=true
    fi
  fi

  if [[ "$should_copy" == true ]]; then
    log "Copying deploy_lab_v13.sh to $remote_target:$target_path"
    scp "${SSH_OPTS[@]}" "$local_v13" "$remote_target:$target_path" || \
      die "Failed to copy deploy_lab_v13.sh to $remote_target:$target_path"
    ssh "${SSH_OPTS[@]}" "$remote_target" "chmod +x '$target_path'" 2>/dev/null || true
  fi
}
...
preflight_access() {
  ...
  if [[ -n "$REMOTE_V13_HOST" ]]; then
    ...
    log "Preflight: checking deploy_lab_v13.sh on remote ($REMOTE_V13_PATH)"
    sync_remote_v13_script "$remote_target" "$REMOTE_V13_PATH" "$remote_home"
  fi
...
run_v13_targeted() {
  ...
  if [[ -z "$target_path" ]]; then
    ...
    sync_remote_v13_script "$normalized" "$target_path" "$remote_home"
...
run_ansible() {
  ...
  # Ensure ANSIBLE_CONFIG points to our project ansible.cfg regardless of cwd.
  export ANSIBLE_CONFIG="$SCRIPT_DIR/ansible.cfg"

  # Auto-activate Python venv if ansible-playbook is not already on PATH.
  if ! command -v ansible-playbook >/dev/null 2>&1; then
    local venv_activate="$SCRIPT_DIR/../venv/bin/activate"
    if [[ -f "$venv_activate" ]]; then
      log "Activating Python venv at $SCRIPT_DIR/../venv"
      # shellcheck disable=SC1090
      source "$venv_activate"
    else
      die "ansible-playbook not found and no venv at $SCRIPT_DIR/../venv/. Install: python3 -m venv ../venv && source ../venv/bin/activate && pip install ansible-core"
    fi
  fi
  need_cmd ansible-playbook

  # Pass automation_profile as extra-var so playbooks use the user's chosen profile
  # instead of the inventory default (ubuntu_r620_kvm).
  local profile_args=()
  if [[ "$PROFILE" != "ubuntu_r620_kvm" ]]; then
    profile_args=(-e "automation_profile=$PROFILE")
  fi

  log "Running ansible-playbook $playbook_rel (profile=$PROFILE)"
  ansible-playbook -i "$INV_FILE" "$pb" "${profile_args[@]}" "${EXTRA_ANSIBLE_ARGS[@]}" "$@"
}
...
run_infra_services() {
  ...
  run_ansible "playbooks/reused/deploy_metallb.yml"
...
run_phase_r620() {
  ...
  run_ansible "playbooks/reused/deploy_metallb.yml" --limit "Host12_1,Host12_2,Host12_3,HostB12_1"
