...
CHR_USER="admin";        CHR_PASS="amolla01"   # MikroTik admin password (set on forced change)
...
export CHR_PASS   # consumed by the expect script as $env(CHR_PASS)
...
# Public key and admin password injected via environment variables
set pubkey    $env(CHR_PUBKEY)
set chr_pass  $env(CHR_PASS)
...
# ─── Login with three-way password handling ───────────────────────────────────
# RouterOS 7 may be in one of three states:
#   A) No password set, first login → forces password change, rejects empty=empty
#      Fix: set chr_pass for both "new password>" prompts
#   B) chr_pass already set (e.g. ER1 after manual fix) → empty password rejected
#      Fix: detect "Login:" again and retry with chr_pass
#   C) Already at CLI (no password required, or logged in from serial)
expect {
    -re {\] >} { }
    -nocase "login:" {
        send "admin\r"
        ...
        expect {
            -re {\] >} {
                # Case C: empty password accepted, at CLI
                set timeout 45
            }
            -re {new password} {
                # Case A: forced password change — set chr_pass (not empty,
                # RouterOS rejects setting the same password as old one)
                send "$chr_pass\r"
                expect -re {repeat}
                send "$chr_pass\r"
                set timeout 45
            ...
            -nocase "login:" {
                # Case B: empty password rejected — chr_pass was already set
                send "admin\r"
                expect -nocase "password:"
                send "$chr_pass\r"
                set timeout 45
...
if [[ "${MS_STATE}" == "not-found" ]]; then
    ...
    # ── Patch Border_Leaf1/2 XMLs for the MonitorSrv-side bridge NICs ────────
    # The OVS bridge br-MS-BL1 needs 2 ports:
    #   Port 1: MonitorSrv's vnet  (just patched above)
    #   Port 2: Border_Leaf1's Ethernet28 vnet  ← MISSING
    # BL1/BL2 have type=bridge NICs for br-MS-BL1/BL2 without OVS virtualport,
    # so libvirt's brctl addif fails silently and the vnets are NOT in OVS.
    # Strategy: patch their XML (for future restarts) AND live-add the already-
    # allocated vnets to OVS right now — no BL restart required.
    info "Patching Border_Leaf1 XML: adding OVS virtualport to br-MS-BL1 NIC..."
    patch_ovs_virtualport Border_Leaf1 br-MS-BL1
    info "Patching Border_Leaf2 XML: adding OVS virtualport to br-MS-BL2 NIC..."
    patch_ovs_virtualport Border_Leaf2 br-MS-BL2

    info "Live-adding BL1/BL2 MonitorSrv vnets to OVS bridges (no restart needed)..."
    for BL_BRIDGE_PAIR in "Border_Leaf1:br-MS-BL1" "Border_Leaf2:br-MS-BL2"; do
        BL_VM="${BL_BRIDGE_PAIR%%:*}"
        BL_BR="${BL_BRIDGE_PAIR##*:}"
        # Get the live vnet name for this bridge from domiflist
        BL_VNET=$(${VIRSH} domiflist "${BL_VM}" 2>/dev/null \
            | awk -v br="${BL_BR}" '$3==br && $1!="-" {print $1; exit}')
        if [[ -z "${BL_VNET}" ]]; then
            warn "${BL_VM}: no live vnet found for ${BL_BR} (VM may need restart)"
        elif sudo ovs-vsctl list-ports "${BL_BR}" 2>/dev/null | grep -qx "${BL_VNET}"; then
            ok "${BL_VM} vnet ${BL_VNET} already in ${BL_BR}"
        else
            sudo ovs-vsctl add-port "${BL_BR}" "${BL_VNET}" 2>&1 \
                && ok "${BL_VM} vnet ${BL_VNET} live-added to ${BL_BR}" \
                || warn "Failed to add ${BL_VNET} to ${BL_BR}"
        fi
    done
