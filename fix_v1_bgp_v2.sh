...
# Using 'sudo virsh console' would prompt for nh1221's password inside expect
# and there is no nh1221 account on MikroTik to confuse with.
...
if [[ "${ER2_BRIDGE_PORTS}" -ge 2 ]]; then
    ...
    if [[ "${ER2_STATE}" == "not-found" ]]; then
        ...
        if [[ -n "${BAD_MAC}" ]]; then
            ...
            # Dump XML, strip the offending <interface type='network'> block, redefine
            ${VIRSH} dumpxml Exit_Router2 > /tmp/er2_clean.xml
            python3 - <<'PYEOF' /tmp/er2_clean.xml "${BAD_MAC}"
import sys, re
path, mac = sys.argv[1], sys.argv[2]
xml = open(path).read()
# Remove the entire <interface type='network'>...</interface> block that contains the bad MAC
...
print(f"Removed block containing {mac}" if xml2 != xml else "Pattern not found — check XML manually")
PYEOF
            ${VIRSH} define /tmp/er2_clean.xml 2>&1 \
                && ok "ER2 XML redefined — corrupt NIC removed" \
                || warn "virsh define failed — check /tmp/er2_clean.xml"
        else
...
if [[ "${MS_STATE}" == "not-found" ]]; then
    ...
    # created (shows in virsh domiflist) but never joins the OVS bridge.
    # Fix: add the virtualport tag so libvirt uses 'ovs-vsctl add-port' instead.
    info "Checking MonitorSrv NIC XML for OVS virtualport tag..."
    ${VIRSH} dumpxml MonitorSrv > /tmp/monitorsrv.xml
    MS_XML_CHANGED=0
    for BRIDGE in br-MS-BL1 br-MS-BL2; do
        # Only patch if virtualport is missing from this interface block
        if ! python3 -c "
import sys, re
xml = open('/tmp/monitorsrv.xml').read()
br = '${BRIDGE}'
# Find the interface block for this bridge
m = re.search(r\"<interface type='bridge'>(?:(?!</interface>).)*?<source bridge='\" + br + r\"'/>(?:(?!</interface>).)*?</interface>\", xml, re.DOTALL)
if m and 'virtualport' not in m.group():
    sys.exit(1)  # needs patching
sys.exit(0)
" 2>/dev/null; then
            info "Adding <virtualport type='openvswitch'/> to ${BRIDGE} NIC..."
            python3 - <<PYEOF /tmp/monitorsrv.xml "${BRIDGE}"
import sys, re
path, bridge = sys.argv[1], sys.argv[2]
xml = open(path).read()
pattern = r"(<source bridge='" + re.escape(bridge) + r"'/>)"
repl = r"\1\n      <virtualport type='openvswitch'/>"
xml2 = re.sub(pattern, repl, xml)
open(path, 'w').write(xml2)
print(f'Patched {bridge} NIC with OVS virtualport')
PYEOF
            MS_XML_CHANGED=1
        else
            info "${BRIDGE} NIC already has OVS virtualport — no change"
        fi
    done
    if [[ "${MS_XML_CHANGED}" -eq 1 ]]; then
        ${VIRSH} define /tmp/monitorsrv.xml 2>&1 \
            && ok "MonitorSrv XML redefined with OVS virtualport" \
            || warn "virsh define failed — check /tmp/monitorsrv.xml"
    fi
