...
# Matches [admin@hostname] > without a $ anchor.
# Anchoring to $ is unreliable on a serial console because RouterOS outputs
# ANSI escape sequences that appear after the > and break the end-of-line match.
proc waitprompt {} {
    expect {
        -re {\] >} { return 0 }
        timeout    { return 1 }
...
# ─── Helper: handle RouterOS forced-password-change flow ─────────────────────
# RouterOS 7 forces a password change on the very first admin login.
# The prompts are:  "new password> "  and  "repeat new password> "
# We accept empty password (just press Enter twice).  RouterOS allows it.
# This is called IMMEDIATELY after sending \r for the login Password: prompt.
# Short timeout (8s) — if neither prompt nor CLI appears, we fall through.
proc handle_password_change {} {
    set timeout 8
    expect {
        -re {\] >}         {
            # Went straight to CLI — no forced change, good
            set timeout 45
            return
        }
        -re {new password}  {
            # RouterOS forcing a password change
            send "\r"
            expect -re {repeat}
            send "\r"
            set timeout 45
        ...
        timeout {
            # Neither appeared — assume we are already at the CLI
            set timeout 45
            return
        }
    }
...
# Handle RouterOS login (no password) or already-at-CLI.
expect {
    -re {\] >}       { }
    -nocase "login:" {
        send "admin\r"
