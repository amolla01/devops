...
VM_BRIDGES[MonitorSrv]="br-MS-BL1,br-MS-BL2"
...
VM_MGMT_IP[HostB12_1]="172.16.2.45"
VM_MGMT_IP[HostB12_2]="172.16.2.46"
VM_MGMT_IP[MonitorSrv]="172.16.2.47"
...
VM_KIND[HostB12_1]="ubuntu"
VM_KIND[HostB12_2]="ubuntu"
VM_KIND[MonitorSrv]="ubuntu"
...
SONIC_STARTUP_PORTS[Border_Leaf1]="Ethernet0 Ethernet20 Ethernet24 Ethernet28 Ethernet120 Ethernet124"
SONIC_STARTUP_PORTS[Border_Leaf2]="Ethernet0 Ethernet20 Ethernet24 Ethernet28 Ethernet120 Ethernet124"
...
SERVER_LOOPBACK_IP[HostB12_1]="10.10.255.100/32"
SERVER_LOOPBACK_IP[HostB12_2]="10.10.255.101/32"
SERVER_LOOPBACK_IP[MonitorSrv]="10.10.255.102/32"
...
SERVER_ROUTER_ID[HostB12_1]="10.10.255.100"
SERVER_ROUTER_ID[HostB12_2]="10.10.255.101"
SERVER_ROUTER_ID[MonitorSrv]="10.10.255.102"
SERVER_BGP_ASN[Host12_1]=65101
SERVER_BGP_ASN[Host12_2]=65102
SERVER_BGP_ASN[Host12_3]=65103
SERVER_BGP_ASN[Host34_1]=65104
SERVER_BGP_ASN[Host34_2]=65105
SERVER_BGP_ASN[HostB12_1]=65106
SERVER_BGP_ASN[HostB12_2]=65107
SERVER_BGP_ASN[MonitorSrv]=65108
...
TARGET_VMS=(
    ...
    HostB12_1 HostB12_2 MonitorSrv
...
reset_sonic_bgp() {
    local vm="$1"
    local ip="${VM_MGMT_IP[$vm]}"
    sonic_cmd "$ip" "docker exec bgp vtysh -c 'clear bgp *'" >/dev/null 2>&1 \
        && ok "$vm BGP sessions reset" \
        || warn "$vm BGP reset failed"
}

wait_for_bgp_settle() {
    local seconds="${1:-45}"
    info "Waiting ${seconds}s for BGP sessions to settle after reset"
    sleep "$seconds"
}
...
disable_tx_offload_for_vm() {
    ...
    local bridge
    while IFS= read -r bridge; do
        [[ -z "$bridge" || "$bridge" == "br-pad" ]] && continue
        local tap
        tap=$(sudo ovs-vsctl list-ports "$bridge" 2>/dev/null | grep '^vnet' | while read -r port; do
            if ${VIRSH} domiflist "$vm" 2>/dev/null | grep -q "^ $port\|^$port"; then
                echo "$port"
                break
            fi
        done)
...
for vm in Spine_S1 Spine_S2 Leaf_L1 Leaf_L2 Leaf_L3 Leaf_L4 Border_Leaf1 Border_Leaf2; do
    bring_up_sonic_ports "$vm"
done
for vm in Host12_1 Host12_2 Host12_3 Host34_1 Host34_2 HostB12_1 HostB12_2 MonitorSrv; do
    disable_tx_offload_for_vm "$vm"
    restart_host_frr "$vm"
    repair_host_frr "$vm"
done

phase "Phase 2.5 - MikroTik edge cleanup"
disable_tx_offload_for_vm Exit_Router1
disable_tx_offload_for_vm Exit_Router2
...
phase "Phase 2.7 - Reset SONiC BGP sessions"
for vm in Spine_S1 Spine_S2 Leaf_L1 Leaf_L2 Leaf_L3 Leaf_L4 Border_Leaf1 Border_Leaf2; do
    reset_sonic_bgp "$vm"
done
wait_for_bgp_settle 45

phase "Phase 3 - OVS bridge counts"
show_bridge_counts

phase "Phase 4 - BGP verification"
for vm in Host12_1 Host12_2 Host12_3 Host34_1 Host34_2 HostB12_1 HostB12_2 MonitorSrv Leaf_L1 Leaf_L2 Leaf_L3 Leaf_L4 Spine_S1 Spine_S2 Border_Leaf1 Border_Leaf2; do
